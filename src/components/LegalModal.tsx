...
export const LegalModal: React.FC<LegalModalProps> = ({ docType, onClose }) => {
  ...
  const legalContent: Record<LegalDocType, {
    ...
    terms: {
      ...
      icon: <FileText className="w-5 h-5 text-[#6FFFE9]" />,
      lastUpdated: 'August 2026 (v4.2.0-ON)',
      sections: [
        {
          heading: '1. Service Scope & Greater Toronto Mechanical Gateway Telemetry',
          body: 'NexusHVAC Systems Corp ("NexusHVAC") provisions cloud-native HVAC telemetry ingestion, ARM cost attribution, and BACnet/IP edge bridging services across commercial real estate portfolios in the Province of Ontario. By accessing this Command Center, mechanical contractors, facilities managers, and engineering operators agree to adhere to these operating terms.'
        },
        {
    ...
    privacy: {
      ...
      icon: <Lock className="w-5 h-5 text-[#6FFFE9]" />,
    ...
    'data-residency': {
      ...
      icon: <Scale className="w-5 h-5 text-[#6FFFE9]" />,
    ...
    cookies: {
      ...
      icon: <Globe2 className="w-5 h-5 text-[#6FFFE9]" />,
      lastUpdated: 'August 2026',
      sections: [
        {
    ...
    msa: {
      ...
      icon: <Award className="w-5 h-5 text-[#6FFFE9]" />,
      ...
      <div className="bg-[#1C2541] border border-[#3A506B] rounded-xl w-full max-w-4xl shadow-[0_0_50px_rgba(111,255,233,0.18)] overflow-hidden flex flex-col max-h-[90vh] text-white font-['Segoe_UI',-apple-system,BlinkMacSystemFont,Roboto,Helvetica,Arial,sans-serif]">
        ...
        <div className="p-6 border-b border-[#3A506B] flex items-start justify-between bg-[#0B132B]">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-lg bg-[#1C2541] text-[#6FFFE9] border border-[#3A506B] shadow-[0_0_15px_rgba(111,255,233,0.2)]">
              {currentDoc.icon}
            </div>
            <div>
              ...
              <p className="text-xs text-[#BCF8EC] mt-1">
                {currentDoc.subtitle} • <span className="font-mono text-[#6FFFE9]">{currentDoc.lastUpdated}</span>
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="text-[#BCF8EC] hover:text-white p-2 rounded-lg bg-[#0B132B] hover:bg-[#142247] border border-[#3A506B] hover:border-[#6FFFE9] transition-all cursor-pointer"
        ...
        <div className="flex items-center gap-1.5 px-6 py-3 bg-[#0B132B] border-b border-[#3A506B] overflow-x-auto">
          {(
            [
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                activeTab === tab.id
                  ? 'bg-[#1C2541] text-[#6FFFE9] border border-[#6FFFE9] shadow-[0_0_12px_rgba(111,255,233,0.2)] font-bold'
                  : 'text-[#BCF8EC] hover:text-white hover:bg-[#1C2541]/50 border border-transparent'
              }`}
            >
              {tab.icon}
              <span>{tab.label}</span>
        ...
        <div className="p-6 sm:p-8 overflow-y-auto space-y-6 text-white text-sm leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="p-3.5 rounded-xl bg-[#0B132B] border border-[#3A506B] flex items-center gap-3">
              <ShieldCheck className="w-5 h-5 text-[#6FFFE9] shrink-0" />
              <div>
                <div className="text-xs font-bold text-white">TSSA Ontario Aligned</div>
                <div className="text-[11px] text-[#BCF8EC]">O. Reg 220/01 Compliant</div>
              </div>
            </div>
            <div className="p-3.5 rounded-xl bg-[#0B132B] border border-[#3A506B] flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[11px] text-[#BCF8EC]">Toronto DC (PIPEDA)</div>
              </div>
            </div>
            <div className="p-3.5 rounded-xl bg-[#0B132B] border border-[#3A506B] flex items-center gap-3">
              <Building2 className="w-5 h-5 text-[#F59E0B] shrink-0" />
              <div>
                <div className="text-xs font-bold text-white">ASHRAE 36 Standard</div>
                <div className="text-[11px] text-[#BCF8EC]">High-Performance BAS</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {currentDoc.sections.map((section, idx) => (
              <div key={idx} className="bg-[#0B132B] p-5 rounded-xl border border-[#3A506B] hover:border-[#6FFFE9] transition-colors space-y-2">
                <h4 className="font-bold text-sm text-[#6FFFE9] flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#6FFFE9]" />
                  {section.heading}
                </h4>
                {Array.isArray(section.body) ? (
                  <ul className="space-y-1.5 pl-4 text-[#BCF8EC] text-xs sm:text-sm list-disc">
                  ...
                  <p className="text-[#BCF8EC] text-xs sm:text-sm leading-relaxed">
                    {section.body}
                  </p>
                )}
              </div>
          ...
          <div className="p-4 rounded-xl bg-[#0B132B] border border-[#3A506B] flex items-start gap-3 text-xs text-[#BCF8EC]">
            <CheckCircle2 className="w-5 h-5 text-[#6FFFE9] shrink-0 mt-0.5" />
            <div>
        ...
        {/* Modal Footer */}
        <div className="p-4 sm:p-5 border-t border-[#3A506B] bg-[#0B132B] flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs text-[#BCF8EC] font-mono">
            Document Hash: <span className="text-[#6FFFE9]">SHA256-NX-ON-{activeTab.toUpperCase()}-2026</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 text-xs font-semibold text-white hover:text-[#6FFFE9] bg-[#1C2541] border border-[#3A506B] hover:border-[#6FFFE9] rounded-lg transition-all flex items-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              onClick={onClose}
              className="px-5 py-2 text-xs font-bold bg-[#6FFFE9] hover:bg-[#5be7d1] text-[#0B132B] rounded-lg shadow-[0_0_15px_rgba(111,255,233,0.3)] transition-all cursor-pointer"
            >
              Accept & Close
            </button>
          </div>
